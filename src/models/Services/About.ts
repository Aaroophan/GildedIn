...
export class AboutService {
    ...
    public async About(Name: string): Promise<any> {
        try {
            ...
            const response = await fetch(`${this.APIURLService.APIURL}/api/about`, {
                method: "POST",
                headers,
            ...
            if (!response.ok) {
                throw new Error(data?.Message || data?.error || `About API request failed with status ${response.status}`)
            }

...
export class SkillsService {
    ...
    public async Skills(Name: string): Promise<SkillsResult> {
        try {
            ...
            const response = await fetch(`${this.APIURLService.APIURL}/api/skills`, {
                method: "POST",
                headers,
            ...
            if (!response.ok) {
                throw new Error(data?.Message || data?.error || `Skills API request failed with status ${response.status}`)
            }
